...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        if self.place is not None:
            ...
            i = 0
            while place_copy.entrance is not None:
                if i < self.lower_bound:
                    place_copy = place_copy.entrance
                    i += 1
                    continue
                elif i > self.upper_bound:
                    break
                if place_copy.bees:
                    return random_bee(place_copy.bees)
                place_copy = place_copy.entrance
                i += 1
            return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in self.place.bees[:]:
            ...
            if self.health - amount <= 0 and bee.health > 0:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        elif not self.turns_to_chew and self.place.bees:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while place is not None:
            if place.ant and place.ant.name != 'Laser':
                insects_and_distance[place.ant] = distance
            if place.bees:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        n = self.damage - 0.25*distance - 0.0625*self.insects_shot
        if n >= 0:
            return n
        else:
            return 0
