def shuffle(s):
    ...
    "*** YOUR CODE HERE ***"
    s0, s1 = s[0 : len(s) // 2], s[len(s) // 2 : len(s)]
    result = []
...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    if total_mass(end(left(m))) * length(left(m)) != total_mass(end(right(m))) * length(
        right(m)
    ):
        return False
    if is_planet(end(left(m))) and is_planet(end(right(m))):
        return True
    if is_mobile(end(right(m))) and is_planet(end(left(m))):
